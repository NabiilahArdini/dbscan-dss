...
Kode di atas berarti kita membuat object bernama `scotty_df` yang berisi data **scotty.csv** dari folder **data**. Tanda `<-` berarti kita menyimpan data kedalam object `scotty_df` sehingga objek tersebut dapat digunakan pada proses analisis selanjutnya. Untuk mengetahui apakah data anda berhasil disimpan bisa lihat panel environment dan lihat objek `scotty_df`. Pada *environment* anda akan melihat objek `scotty_df` berisi 124489 baris dan 10 kolom.
...
data di atas merupakan data ride-sharing asal Turki yang bernama scotty. Deskripsi dari setiap kolom sebagai berikut:      
...
Tipe data Date dan Time merupakan tipe data yang cukup esensial pada kasus pemesanan ride-sharing, selain 4 tipe data dasar di atas R memiliki tipe data lainnya yaitu date dan datetime. Pada `scotty_df` terdapat kolom `start_time` yang menunjukkan waktu pemesanan pengguna. 
...
arti dari kode di atas yaitu dari data `scotty_df` kita membuat kolom yang bernama `start_time` yang berisi hasil transformasi kolom `start_time` menjadi format datetime (POSIXct).

Kita bisa mengetahui rentang waktu pengambilan data dengan menggunakan fungsi `range()`.
```{r}
range(scotty_df$start_time)
```
Dari hasil di atas kita tahu bahwa fata penggunaan ride sharing diperoleh selama bulan November 2017. Ada banyak informasi yang bisa kita ambil dari kolom yang bertipe data datetime seperti nama hari, jam, dll. Hal tersebut akan kita lakukan pada bagian selanjutnya. 
...
Code di atas membuat 3 kolom baru (date, dow, hour) yang mengekstrak informasi dari `start_time`. Berikut penjelasan detail terkait fungsi di atas: 
...
seluruh fungsi di atas harus didalam fungsi `mutate()` agar kolom baru terbentuk. Setelah itu hasil data.frame baru disimpan pada objek baru yang bernama `scotty_clean`.
...
Dari hasil di atas diketahui terdapat 5666 data yang tidak mendapatkan driver ketika memesan pada aplikasi, data ini yang akan menjadi fokus amantan kita. Sekarang kita cari tahu kapan paling sering terjadi `nodrivers`. 

Sebelum mencari tau kapan `nodrivers` sering terjadi kita bisa melakukan subsetting data terlebih dahulu. Kita bisa menggunakan fungsi `filter()` untuk mengambil data yang berstatus `nodrivers` saja.
...
Dari hasil aggregasi di atas kita tahu bahwa pada jam 15 sampai 19 kasus `nodrivers` sering ditemui. Untuk mempermudah proses pemgambilan keputusan hasil di atas bisa kita visualisasikan.
...
Visualisasi di atas menampilkan frekuensi `nodrivers` untuk setiap jamnya, garis merah horizontal menunjukkan nilai rata rata dari frekuensi. Bila kita amati jam 8 dan jam 15 hingga 19 merupakan jam dengan kondisi `nodrivers` di atas rata rata, terutama pada jam 18. Dari informasi yang sudah kita temukan fokus amatan pada analisis kali ini adalah kondisi `nodrivers` yang terjadi di hari jumat pada jam 15 sampai 19. 
...
Data yang digunakan sebagai ilustrasi pada clustering ini adalah data `multishape` dari packages `factoextra`. Data `multishape` memiliki 1100 observasi dan 3 variabel yaitu `x`, `y`, dan `shape`. Variabel yang digunakan pada proses clustering ini adalah `x` dan `y`.
...
Terdapat 2 parameter yang harus di tentukan sebelum membuat cluster yaitu eps dan Minpts. Tidak ada algoritma atau metode yang dapat menentukan secara sempurna kedua nilai tersebut karena itu semua bergantung pada jumlah dataset dan kasus yang diamati. Namun sebagai acuan awal kita bisa menggunakan fungsi `kNNdistplot()` dari packages `dbscan` untuk mencari kombinasi nilai minpts dan eps .

Ide utama dari fungsi `kNNdistplot()` adalah menghitung jarak rata2 untuk setiap data ke k tetangga terdekatnya (nearest neighbors). Nilai dari K ditentukan oleh user yang nantinya akan digunakan sebagai minPts pada proses clustering. Rata rata jarak yang sudah didapat divisualisasikan dalam plot secara ascending untuk mendapatkan "knee" yang menunjukkan nilai optimal dari eps berdasarkan K yang ditentukan.
...
Berdasarkan plot di atas dengan menggunakan K = 4 didapat jarak yang optimal yaitu sekitar 0.15. Nilai 0.15 didapat dari posisi "knee" yang terbentuk pada plot. Hasil pencarian nilai eps yang optimal di atas dapat digunakan dalam proses clustering yang mana nilai eps adalah 0.15 dengan minPts 4. Tahap selanjutnya adalah pembuatan cluster menggunakan function dbscan dengan parameter yang sudah didapat.
...
Dari hasil visualisasi di atas bisa dilihat bahwa cluster yang dihasilkan dbscan dapat mengelompokkan data sehingga bisa menangkap beberapa bentuk dari data. Noise yang terdeteksi pada data terlihat menyebar, hal ini dikarenakan data noise tidak berhasil ditangkap oleh eps yang ditentukan.
...
Dari hasil visualisasi di atas bisa dilihat bahwa cluster 1 yang baru menggabungkan cluster 1 dan 2 pada proses cluster sebelumnya, serta noise yang awalnya berada disekitar cluster 1 menjadi anggota cluster 1.

# Implementasi DBSCAN

## Data Preparation

Pada bagian sebelumnya kita sudah mempelajari metode DBSCAN, dan sudah melakukan eksplorasi data pada data scotty. Sekarang kita akan menerapkan DBSCAN untuk membuat cluster pada data scotty. Data scotty merekam banyaknya demand yang diterima oleh perusahaan pada waktu tertentu dan dari beragam lokasi (longitude dan latitude). Jumlah demand yang tinggi dan menyebar dapat menyulitkan perusahaan untuk mengetahui titik rawan mana yang banyak mengalami kondisi "no drivers" atau yang hanya berupa *noise*. Clustering pada data scotty akan membantu kita melihat menentukan lokasi rawan tersebut agar perusahaan dapat melakukan peningkatan layanan dengan lebih optimal.

Dari hasil eksplorasi sebelumnya kita mengetahui bahwa status `nodrivers` sering terjadi pada hari jumat di jam 15 sampai 19. Mari kita lihat jumlah `nodrivers` pada setiap tanggal dengan kondisi tersebut.
...
Hasil di atas menunjukkan bahwa pada tanggal 2017-11-03 merupakan kasus `nodrivers` tertinggi, tanggal tersebut yang akan menjadi amatan kita dalam proses pembuatan cluster oleh sebab itu kita perlu mengambil data pada tanggal 2017-11-03 di jam 15 hingga 19.
...
fungsi `leaflet()` digunakan sebagai dasar dari map yang dibuat, kemudian untuk menambahkan basemap kita bisa gunakan fungsi `addProviderTiles()`. Ada banyak penyedia basemap (providers) kita bisa memilihnya sesuai kebutuhan, disini provider yang digunakan adalah `CartoDB.Positron` agar terlihat kontras dengan data point-nya. Terakhir kita bisa menambahkan markers untuk setiap data dengan fungsi `addCircleMarkers()`, masukkan longitude dan latitude sebagai parameter dan radius untu mengatur besar kecilnya marker. tanda `~` pada `~src_lon` menunjukkan bahwa kita akan menggunakan kolom `src_lon` dari data `scotty_final`.

## Clustering

Setelah mengetahui persebaran data melalui peta, kita bisa mulai membuat cluster dengan menentukan nilai eps dan minpts terlebih dahulu dengan bantuan knee plot. 
...
Pemilihan minpts (k) 9 menghasilkan nilai eps yang optimal sekitar 0.02, kita akan mencoba memasukkan kedua nilai tersebut sebagai parameter dalam fungsi `dbscan()` dan melihat hasil cluster yang terbentuk.
...
Hasil clustering dengan menggunakan parameter eps = 0.02 dan minpts = 9 adalah terbentuknya 2 cluster, dimana cluster 1 berisi 797 data (sekitar 84% data). Hasil cluster yang terlalu besar bisa menjadi salah satu tanda proses clustering belum optimal, oleh sebab itu kita dapat mengecilkan nilai eps menjadi 0.01 agar cluster yang terbentuk lebih rapat.
...
Cluster yang terbentuk dengan menggunakan eps 0.01 dan minpts 9 adalah 9 cluster. Selain 9 cluster tersebut terdapat 242 data yang tidak masuk cluster manapun atau bisa disebut sebagai noise. Hasil cluster di atas dapat divisualisasikan pada peta namun sebelumnya label cluster tersebut perlu kita ambil dan memasukkannya pada data `scotty_final`. 
...
Sekarang setiap data sudah memiliki label clusternya masing masing, agar lebih mudah membedakan tiap cluster kita bisa memberikan warna untuk setiap cluster. Salah satu cara pewarnaan yang paling mudah adalah dengan menggunakan palette color yang disediakan oleh package `pals`. Dibawah ini kita membuat palette color berdasarkan jumlah cluster yang terbentuk.
...
Refrensi warna di atas bisa kita gabungkan dengan data scotty_final dengan menggunakan fungsi `left_join()` sehingga pada data scotty_final akan terbentuk kolom baru yaitu col. 
...
Visualisasi di atas menampilkan 9 cluster yang diwakili tiap warna tanpa menampilkan noise. Daerah tempat cluster terbentuk merupakan daerah yang memiliki kondisi "no drivers" cukup tinggi di hari jumat pada pukul 15 hingga 19. Jika diperhatikan cluster 1 (biru tua) merupakan cluster terbesar, hal ini terjadi karena kerapatan didaerah tersebut lebih rapat dibandingkan dengan daerah lain.

Solusi untuk cluster memecah cluster yang besar adalah dengan melakuan clustering ulang pada cluster 1 saja, sehingga nilai ukuran kerapatannya (eps dan minpts) tidak mengganggu cluster yang sudah terbentuk.

Dari hasil cluster di atas, perusahaan ride sharing dapat memfokuskan alokasi mitra driversnya untuk berada pada lokasi cluster tersebut sehingga kejadian "no drivers" bisa dikurangi.

## Penerapan lainnya pada DBSCAN

DBSCAN sering digunakan pada pengelompokkan data spatial karena sifatnya yang mengelompokkan data berdasarkan kerapatannya. Selain itu DBSCAN juga dapat dimanfaatkan untuk menghilangkan noise pada data sebelum data digunakan pada proses analisa selanjutnya. Berikut beberapa penerapan DBSCAN:        
1. [Spatial Clustering Berbasis Densitas untuk Persebaran Titik Panas sebagai Indikator Kebakaran Hutan dan Lahan Gambut di Sumatera](
https://repository.ipb.ac.id/handle/123456789/73111)            
2. [Hotspot Detection of Dengue Fever Outbreaks Using DBSCAN Algorithm](https://ieeexplore.ieee.org/abstract/document/8776916)              
3. [Clustering Saham Indonesia](https://algotech.netlify.app/blog/stock-cluster/)           